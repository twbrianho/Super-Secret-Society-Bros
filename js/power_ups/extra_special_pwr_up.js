class ExtraSpecialPwrUp {
    constructor(player1, player2) {
        ...
        this.pwrBoxes.createMultiple(50, 'extraSpecialPwrUp');
    ...
    checkCollisions(player1, player2) {
        game.physics.arcade.overlap(player1.sprite, this.pwrBoxes, function (player, pwrBox) {
            if (player.playerClassParent.specials < 3) {
                if (player.playerClassParent.specials === 0)  {
                    // revive player
                    player.alpha = 1;
                    player.vulnerable = true;
                }
                player.playerClassParent.specials++;
                pwrBox.kill();
                SFX_EXTRA_SPECIAL.play();
            }
        });
